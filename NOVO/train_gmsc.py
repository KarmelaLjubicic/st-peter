...
##
data = pd.read_csv("gmscredit_dataset/cs-training.csv", index_col=0)#[:15000]
...
def batch_pairs(i, perm_size, residue, epochs, perm, m, good, bad):
    p = i * perm_size
    r = (i + 1) * perm_size
    if (residue != 0 and i + 1 == epochs) or perm_size == 0:
        r = p + residue
    perm_batch = perm[p:r]
...
for i in range(epochs):
    ...
    while True:
        try:
            M.sess.run(M.train_op, feed_dict={M.learning_rate: 1e-4, M.batch_size: len(concatenated_pairs),
    ...
    '''if i in [100, 350, 600, 850]:
        validate_test(perm_v_size, residue_v, epochs, perm_v, m_v, X_valid_good, X_valid_bad)'''
    if i % 100 == 0:
